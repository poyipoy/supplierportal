...
- **Date Pickers & Calendars:** Always use the custom design system calendar components `<x-ui.date-picker>` (for single date) and `<x-ui.date-range-picker>` (for date ranges). **NEVER** use browser-native `<input type="date">` in any form, modal, or filter (inconsistent across browsers/OS and breaks design system). Always provide a unique `id` attribute when using inside loops or modals.
