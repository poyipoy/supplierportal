...
class LoginRequest extends FormRequest
{
     ...
     */
    public function authenticate(LoginRateLimiter $limiter, TurnstileVerifier $turnstile, SsoDomainResolver $ssoResolver): User
    {
        $email = $limiter->normalizedEmail($this->string('email')->toString());

        $ssoConnection = $ssoResolver->resolve($email);

        if ($ssoConnection && $ssoConnection->enforce_sso) {
            event(new AuthSecurityEvent('sso_login_blocked', email: $email, metadata: ['reason' => 'enforce_sso_domain']));

            throw ValidationException::withMessages([
                'email' => 'Your organization requires signing in through company SSO. Use the "Sign in with company SSO" option below.',
            ]);
        }
