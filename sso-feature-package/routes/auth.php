...
Route::middleware('guest')->group(function () {
    ...
    Route::get('login/sso', [SsoAuthController::class, 'show'])
        ->middleware('no-store')->name('sso.show');

    Route::post('login/sso', [SsoAuthController::class, 'redirect'])
        ->middleware(['throttle:auth.credentials', 'no-store'])->name('sso.redirect');
...
/*
|--------------------------------------------------------------------------
| SSO callbacks (SAML ACS / SLS, OIDC redirect URI)
|--------------------------------------------------------------------------
| These are reached directly by the supplier's identity provider, so they
| must stay outside the `guest` middleware group (the browser may still
| carry an old/expired session cookie) but remain `no-store` like the rest
| of the auth flow.
*/
Route::post('auth/sso/saml/{ssoConnection}/acs', [SsoAuthController::class, 'samlAcs'])
    ->middleware('no-store')->name('sso.saml.acs');
Route::get('auth/sso/saml/{ssoConnection}/sls', [SsoAuthController::class, 'samlSls'])
    ->middleware('no-store')->name('sso.saml.sls');
Route::get('auth/sso/oidc/{ssoConnection}/callback', [SsoAuthController::class, 'oidcCallback'])
    ->middleware('no-store')->name('sso.oidc.callback');
