        ...
        <form id="ruleForm" method="POST" action="{{ route('admin.hs-code-rules.store') }}" class="modal-content">
            ...
            <div class="modal-body">
                <div class="row g-3 mb-4">
                    <div class="col-md-3"><label class="form-label">HS Code</label><input type="text" name="hs_code" id="ruleHsCode" class="form-control" placeholder="7228.30.10" required value="{{ old('form_context') === 'rule' ? old('hs_code') : '' }}"></div>
                    <div class="col-md-3"><label class="form-label">Category</label><select name="material_category" id="ruleCategory" class="form-select" required>@foreach($hsCategories as $category)<option value="{{ $category }}">{{ str_replace('_', ' ', $category) }}</option>@endforeach</select></div>
                    ...
                    <div class="col-md-2"><label class="form-label">Priority</label><input type="number" name="priority" id="rulePriority" class="form-control" min="1" max="65535" value="100" required></div>
                    <div class="col-md-2"><label class="form-label">Status</label><select name="status" id="ruleStatus" class="form-select"><option value="active">Active</option><option value="inactive">Inactive</option><option value="conflict">Conflict</option></select></div>
                </div>
