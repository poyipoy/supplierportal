...
@section('page-title', __('Buat Permintaan Material Baru'))

@section('content')
<div class="card border-0 shadow-sm">
    <div class="card-header bg-white py-3">
        <h5 class="mb-0 fw-semibold">{{ __('Formulir Permintaan Material') }}</h5>
    </div>
    <div class="card-body">
        <form id="prForm" action="{{ route('purchasing.requirements.store') }}" method="POST">
            ...
            <div class="row mb-4">
                <div class="col-md-6">
                    <label for="period_id" class="form-label fw-medium">{{ __('Periode Penawaran') }} <span class="text-danger">*</span></label>
                ...
                <div class="col-md-6">
                    <label for="notes" class="form-label fw-medium">{{ __('Catatan / Keterangan Tambahan') }}</label>
                    <textarea name="notes" id="notes" class="form-control" rows="2" placeholder="{{ __('Opsional...') }}">{{ old('notes') }}</textarea>
                </div>
            </div>
            ...
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h6 class="fw-bold mb-0">{{ __('Daftar Material yang Dibutuhkan') }}</h6>
                <button type="button" class="btn btn-sm btn-success" id="btnAddRow">
                    <i class="bi bi-plus"></i> {{ __('Tambah Material') }}
                </button>
            </div>
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-sm align-middle" id="itemsTable">
                    <thead class="table-light text-center" style="font-size: 0.8rem;">
                        <tr>
                            <th width="10%">{{ __('HS Code') }}</th>
                            <th width="18%">{{ __('Material Name') }} <span class="text-danger">*</span></th>
                            <th width="12%">{{ __('Shape') }}</th>
                            <th width="8%">{{ __('Thickness') }}</th>
                            <th width="8%">{{ __('Inner Dia.') }}</th>
                            <th width="8%">{{ __('Outer Dia.') }}</th>
                            <th width="8%">{{ __('Width') }}</th>
                            <th width="8%">{{ __('Length') }}</th>
                            <th width="12%">{{ __('Weight (Kg)') }} <span class="text-danger">*</span></th>
                ...
                <div id="noItemAlert" class="text-danger small mt-1 d-none">{{ __('Minimal harus ada 1 material.') }}</div>
            </div>

            <div class="mt-4 d-flex gap-2">
                <a href="{{ route('purchasing.requirements.index') }}" class="btn btn-light">{{ __('Batal') }}</a>
                <button type="button" class="btn btn-secondary" onclick="submitForm('draft')">{{ __('Simpan Draft') }}</button>
                <button type="button" class="btn btn-primary" style="background-color: var(--adasi-blue);" onclick="confirmSubmit()">{{ __('Ajukan Sekarang') }}</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    function removeRow(btn) {
        Swal.fire({
            title: @json(__('Hapus baris ini?')),
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#d33',
            cancelButtonColor: '#3085d6',
            confirmButtonText: @json(__('Ya')),
    ...
    function submitForm(action) {
        if ($('#itemsBody tr').length === 0) {
            $('#noItemAlert').removeClass('d-none');
            Swal.fire(@json(__('Error')), @json(__('Minimal 1 material wajib ditambahkan.')), 'error');
            return;
        }
