...
@section('page-title', __('Edit Permintaan Material'))

@section('content')
<div class="card border-0 shadow-sm">
    <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
        <h5 class="mb-0 fw-semibold">{{ __('Edit Formulir Permintaan Material') }}</h5>
        @if($pr->status === 'rejected')
            <span class="badge bg-danger">{{ __('Status Saat Ini') }}: {{ __('Rejected') }} ({{ __('Mohon Direvisi') }})</span>
        @else
            <span class="badge bg-secondary">{{ __('Status Saat Ini') }}: {{ __('Draft') }}</span>
        @endif
    </div>
    <div class="card-body">
        <form id="prForm" action="{{ route('purchasing.requirements.update', $pr->id) }}" method="POST">
            ...
            <div class="mt-4 d-flex gap-2">
                ...
                <button type="button" class="btn btn-primary" style="background-color: var(--adasi-blue);" onclick="confirmSubmit()">
                    {{ $pr->status === 'rejected' ? __('Revisi & Ajukan Ulang') : __('Ajukan Sekarang') }}
                </button>
            </div>
        </form>
    </div>
