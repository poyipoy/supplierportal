...
@section('page-title', __('Negosiasi dengan Purchasing'))

@section('content')
    <div class="card border-0 shadow-sm">
        <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
            <h6 class="mb-0 fw-bold">{{ __('Daftar Chat Negosiasi') }}</h6>
        </div>
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-hover align-middle datatable" style="font-size: 0.9rem;">
                    <thead class="table-light">
                        <tr>
                            <th>{{ __('Konteks Dokumen') }}</th>
                            <th>{{ __('Purchasing Officer') }}</th>
                            <th>{{ __('Pesan Terakhir') }}</th>
                            <th>{{ __('Waktu Terakhir') }}</th>
                    ...
                    <tbody>
                        @forelse($conversations as $conv)
                            <tr>
                                ...
                                <td>
                                    ...
                                    @else
                                        <em class="text-muted">{{ __('Belum ada pesan') }}</em>
                                    @endif
                                </td>
                                <td>
                                    @if($conv->latestMessage)
                                ...
                                <td class="text-end">
                                    ...
                                    <a href="{{ route('supplier.conversations.show', $conv->id) }}" class="btn btn-sm btn-outline-primary position-relative" data-open-chat-conversation="{{ $conv->id }}">
                                        <i class="bi bi-chat-text"></i> {{ __('Buka Chat') }}
                                        @if($unreadCount > 0)
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="text-center py-4 text-muted">{{ __('Belum ada riwayat percakapan/negosiasi.') }}</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
