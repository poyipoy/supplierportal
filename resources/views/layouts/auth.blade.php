...
<head>
    ...
    <title>@yield('title', __('Login') . ' - ADASI Supplier Portal')</title>
...
<body>
    <div class="auth-language-switcher d-flex gap-1">
        <form action="{{ route('language.switch', 'id') }}" method="POST">
            @csrf
            <button type="submit" class="btn btn-sm {{ session('locale', 'id') === 'id' ? 'btn-primary' : 'btn-outline-secondary' }}">&#x1F1EE;&#x1F1E9; ID</button>
        </form>
        <form action="{{ route('language.switch', 'en') }}" method="POST">
            @csrf
            <button type="submit" class="btn btn-sm {{ session('locale', 'id') === 'en' ? 'btn-primary' : 'btn-outline-secondary' }}">&#x1F1EC;&#x1F1E7; EN</button>
        </form>
    </div>
