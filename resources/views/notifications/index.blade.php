...
@section('page-title', __('Notifikasi'))

@section('content')
<div class="card border-0 shadow-sm">
    <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
        <h5 class="mb-0 fw-bold">{{ __('Semua Notifikasi') }}</h5>
        <form action="{{ route('notifications.mark-all-read') }}" method="POST">
            @csrf
            <button type="submit" class="btn btn-sm btn-outline-primary"><i class="bi bi-check2-all me-1"></i>{{ __('Tandai Semua Dibaca') }}</button>
        </form>
    </div>
    <div class="card-body p-0">
        <div class="list-group list-group-flush">
                    ...
                    <div class="d-flex gap-3 align-items-start">
                        ...
                        <div style="flex:1">
                            <div class="fw-bold small d-flex justify-content-between">
                                ...
                                @if(!$notif->read_at)<span class="badge bg-danger" style="font-size:.55rem">{{ __('Baru') }}</span>@endif
                            </div>
            ...
            @empty
                <div class="p-5 text-center text-muted"><i class="bi bi-bell-slash" style="font-size:3rem"></i><p class="mt-3">{{ __('Belum ada notifikasi.') }}</p></div>
            @endforelse
        </div>
    </div>
