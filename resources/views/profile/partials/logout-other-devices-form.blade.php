    ...
    <form method="POST" action="{{ route('profile.logout-other-devices') }}">
        ...
        <div class="tw-grid tw-gap-1.5 tw-mb-4">
            ...
            <input id="logout_other_devices_password" name="password" type="password"
                class="tw-h-10 tw-w-full tw-rounded-ui-sm tw-border tw-bg-surface tw-px-3 tw-text-ui-sm tw-text-on-surface focus:tw-border-primary focus:tw-ring-2 focus:tw-ring-primary {{ $errors->logoutOtherDevices->has('password') ? 'tw-border-error' : 'tw-border-outline-variant' }}"
