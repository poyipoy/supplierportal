...
class LocalProcurementController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->filled('q')) $query->where('po_number', 'like', '%'.addcslashes($request->string('q'), '%_').'%');
        if ($supplier) $query->where('supplier_id', $supplier->id);
        if ($request->filled('status')) $query->where('status', $request->string('status'));
        if ($request->filled('date_from')) $query->whereDate('po_date', '>=', $request->string('date_from'));
        if ($request->filled('date_to')) $query->whereDate('po_date', '<=', $request->string('date_to'));
    ...
    {
        $service->closePurchaseOrder($request->user(), $purchaseOrder); return back()->with('success', 'Local PO closed.');
    }
    public function cancel(Request $request, LocalPurchaseOrder $purchaseOrder, LocalProcurementMasterService $service)
    {
        $service->cancelPurchaseOrder($request->user(), $purchaseOrder); return back()->with('success', 'Local PO cancelled.');
    }
    ...
    {
        $service->createGoodsReceipt($request->user(), $purchaseOrder, $request->validated()); return back()->with('success', 'Goods Receipt added.');
    }
    ...
    {
        $service->updateGoodsReceipt($request->user(), $goodsReceipt, $request->validated()); return back()->with('success', 'Goods Receipt updated.');
    }
    public function cancelGoodsReceipt(Request $request, LocalGoodsReceipt $goodsReceipt, LocalProcurementMasterService $service)
    {
        $service->cancelGoodsReceipt($request->user(), $goodsReceipt); return back()->with('success', 'Goods Receipt cancelled.');
    }
    public function template() { return Excel::download(new LocalPoGrImportTemplateExport(), 'local-po-gr-template.xlsx'); }
    ...
    private function suppliers() { return User::localEligible()->with('supplier')->orderBy('name')->get(); }
    ...
    private function prefix(): string { return request()->routeIs('purchasing.*') ? 'purchasing.local-procurement' : 'finance.local-procurement'; }
}
