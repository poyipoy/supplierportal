...
class PaymentBatchTransferExport implements GeneratesWorkbook, TracksExportProgress
{
    ...
    /**
     * Generate the transfer spreadsheet and save to storage.
     */
    public function generateWorkbook(string $path, string $disk): void
    {
        ...
        if ($batches->isEmpty()) {
            throw new RuntimeException('No eligible supplier payment batches found for transfer export.');
        }

        // Validate every requested batch has active supplier items
        foreach ($batches as $batch) {
            ...
            if (! $hasActiveItems) {
                throw new RuntimeException(
                    "Batch [{$batch->batch_number}] contains no active supplier payment items for transfer export."
                );
            }
        }

        // Validate bank mappings for all groups before generating
        ...
        if (! empty($bankFailures)) {
            ...
            throw new RuntimeException(
                "Bank mapping validation failed for transfer export. Unresolvable banks: {$details}"
            );
        }

        $renderer = app(PaymentBatchTransferSheetRenderer::class);
        $spreadsheet = $renderer->render($batches);

        $tempPath = tempnam(sys_get_temp_dir(), 'drp_transfer_');
        if ($tempPath === false) {
            throw new RuntimeException('Failed to create temporary file for transfer export.');
        }
    ...
    /**
     * Eager-load the payment batches with active, non-cancelled data
     * including supplier profile for PIC email.
     *
    ...
    private function authorizeActor(): void
    {
        ...
        if ($actor === null || (! $actor->isFinance() && ! $actor->isAdmin())) {
            throw new RuntimeException('Unauthorized actor for DRP transfer export.');
        }
    }
}
